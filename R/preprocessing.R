...
preprocess <- function(x, y, t, unit = "px", res, dim, dist, fr, blink = NULL, b.win = 0.05,
  ...
  direc[valid] <- calc_direction_deviation(x.va[valid], y.va[valid], t[valid], sampling_rate = fr)

  valid[valid] <- ifelse(is.na(direc[valid]), F, valid[valid])
  ...
  output <- data.frame(frame = 1:length(x), x = x.va, y = y.va, t = t, vel, acc, direc, label) # [valid,]
